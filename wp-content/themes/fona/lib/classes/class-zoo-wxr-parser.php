...
class ZooWXRParser
{
   ...
   * Constructor
   *
   * @param  string  $file  Full path to XML file.
   */
  ...
  function parse($file)
  {
    ...
    if (empty($file)) {
      return new WP_Error('WXR_parse_error', esc_html__('Invalid WXR file.', 'fona'));
    }
    ...
    if (extension_loaded('simplexml')) {
      ...
      return new WP_Error('WXR_parse_error', esc_html__('No XML parser found.', 'fona'));
    }
  }
  ...
  protected function simplexmlParse($file)
  {
    ...
    if (!$xml) {
      return new WP_Error('WXR_parse_error', esc_html__('Invalid WXR file.', 'fona'), libxml_get_errors());
    }
  ...
  protected function xmlreaderParse($file)
  {
    ...
    $reader = new XMLReader();
		...
		if (!$status) {
			return new WP_Error('WXR_parse_error', esc_html__('Could not open the file for parsing', 'fona'));
		}
