...
class ZooDemoImporter
{
	 ...
	 */
  	function import($file)
  	{
		...
		if (!empty($content['users'])) {
			$this->importUsers($content['users']);
		}
	 ...
	 * Import users
	 */
	function importUsers(array $users)
  	{
		...
		foreach ($users as $user) {
			...
			$userdata = array(
				...
				'role'         => 'subscriber',
				'rich_editing' => false,
				'description'  => esc_html__('This user is created while installing demo content. You should delete or modify this user&#8217;s information now.', 'fona')
			);
        ...
        if (!empty($customize_data)) {
            ...
            if (!empty($customize_data['options'])) {
                foreach ($customize_data['options'] as $option_key => $option_value) {
                    $option = new ZooCustomizeSetting($wp_customize, $option_key, array('type' => 'option'));
     ...
     * Import settings
     */
    function importThemeSettings($base_path)
    {
        ...
        if (file_exists($base_path . 'sensei-settings.json') && function_exists('Sensei')) {
            ...
            if ($sensei_settings) {
                ...
                if (isset($imported_posts[$sensei_settings['course_page']])) {
                    $sensei_settings['course_page'] = $imported_posts[$sensei_settings['course_page']];
                }
                if (isset($imported_posts[$sensei_settings['my_course_page']])) {
                    $sensei_settings['my_course_page'] = $imported_posts[$sensei_settings['my_course_page']];
                }
	...
	protected function importPostAttachment($post, $meta, $remote_url)
	{
		...
		if (!$info) {
			return new WP_Error('attachment_processing_error', esc_html__('Invalid attachment file type!', 'fona'));
		}
		...
		foreach ($post['meta'] as $meta) {
			...
			if ($meta['key']) {
                ...
                if (('clever_menu_theme' === $post['post_type']) && class_exists('Clever_Mega_Menu_Theme_Meta', false) && (Clever_Mega_Menu_Theme_Meta::META_KEY === $meta['key'])) {
                    ...
                    $theme_meta->generate_css($value ? $value : array(), $theme_post);
                }
			}
		}
	}
	...
	protected function fetchRemoteFile($url, $post)
	{
		...
		if (200 !== $code) {
			unlink($upload['file']);
			return new WP_Error('import_file_error', sprintf(esc_html__('Remote server returned %1$d %2$s for %3$s', 'fona'), $code, get_status_header_desc($code), $url));
		}
		...
		if (isset($headers['content-length']) && $filesize !== (int) $headers['content-length']) {
			unlink($upload['file']);
			return new WP_Error('import_file_error', esc_html__('Remote file is incorrect size', 'fona'));
		}

		if (0 === $filesize) {
			unlink($upload['file']);
			return new WP_Error('import_file_error', esc_html__('Zero size file downloaded.', 'fona'));
		}
		...
		if (!empty($max_size) && $filesize > $max_size) {
			unlink($upload['file']);
			$message = sprintf(esc_html__('Remote file is too large, limit is %s.', 'fona'), size_format($max_size));
	...
	protected function postExists(array $post)
  	{
		...
		if (null === $exists) {
			global $wpdb;
			$exists = array();
			$db_posts = $wpdb->get_results($wpdb->prepare("SELECT ID, guid FROM {$wpdb->posts}", ''));
	...
	protected function commentExists($comment)
	{
		...
		if (null === $exists) {
			global $wpdb;
			$exists = array();
			$db_comments = $wpdb->get_results($wpdb->prepare("SELECT comment_ID, comment_author, comment_date FROM {$wpdb->comments}", ''));
	...
	protected function termExists(array $term)
	{
		...
		if (null === $exists) {
			global $wpdb;
			$exists = array();
			$db_terms = $wpdb->get_results($wpdb->prepare("SELECT t.term_id, tt.taxonomy, t.slug FROM {$wpdb->terms} AS t JOIN {$wpdb->term_taxonomy} AS tt ON t.term_id = tt.term_id", ''));
	...
	protected function register_custom_taxonomy($taxonomy, $object_type, $args = array())
	{
		...
		if ( empty($taxonomy) || strlen( $taxonomy ) > 32 ) {
	  		_doing_it_wrong( __FUNCTION__, esc_html__( 'Taxonomy names must be between 1 and 32 characters in length.', 'fona' ), '4.2.0' );
	  		return new WP_Error( 'taxonomy_length_invalid', esc_html__( 'Taxonomy names must be between 1 and 32 characters in length.', 'fona' ) );
		}
