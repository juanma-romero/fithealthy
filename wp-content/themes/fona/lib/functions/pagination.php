...
if (!function_exists('zoo_ajax_pagination'))
{
    function zoo_ajax_pagination(WP_Query $query = null, array $args = array())
    {
        ...
        $args = array(
            ...
            'prev_text'            => !empty($args['prev_text']) ? $args['prev_text'] : esc_html__('Prev Posts', 'fona'),
            'next_text'            => !empty($args['next_text']) ? $args['next_text'] : esc_html__('Next Posts', 'fona'),
            'horizontal_alignment' => '',
            'gutter'               => '',
            'column_width'         => '',
            'more_text'            => !empty($args['more_text']) ? $args['more_text'] : apply_filters('zoo_ajax_pagination_more_text', esc_html__('Load More', 'fona')),
            'no_more_text'         => !empty($args['no_more_text']) ? $args['no_more_text'] : apply_filters('zoo_ajax_pagination_no_more_text', esc_html__('No More Posts', 'fona')),
        );
        ...
        echo '<div id="spinner-'.esc_attr($uid).'" style="text-align:center;display:none">' . apply_filters('zoo_ajax_pagination_spinner', '<img aria-hidden="true" role="presentation" src="'.ZOO_THEME_URI.'lib/assets/icons/spinner.svg'.'" width="48" height="48">') . '</div>';
