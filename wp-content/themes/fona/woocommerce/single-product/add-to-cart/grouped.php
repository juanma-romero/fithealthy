...
global $product, $post;
remove_action('zoo_woo_single_meta', 'woocommerce_template_single_meta', 10);
...
<form class="cart  grouped_form" action="<?php echo esc_url( get_permalink() ); ?>" method="post" enctype='multipart/form-data'>
    <table cellspacing="0" class="woocommerce-grouped-product-list group_table">
        ...
        foreach ( $grouped_products as $grouped_product ) {
            ...
            echo '<tr id="product-' . esc_attr( get_the_ID() ) . '" class="woocommerce-grouped-product-list-item ' . esc_attr( implode( ' ',  wc_get_product_class( '', $grouped_product->get_id() ) ) ) . '">';
            ...
            foreach ( $grouped_product_columns as $column_id ) {
                ...
                switch ( $column_id ) {
                    ...
                    case 'label':
                        ...
                        break;
                    case 'image':
                        $value = get_the_post_thumbnail($grouped_product->get_id(),array(80,80));
                        break;
                    default:
                        $value = '';
                        break;
	...
	<?php
    echo wc_get_template_part('single-product/meta');
