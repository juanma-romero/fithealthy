...
class Kirki_Autoload {
	...
	protected function get_paths( $class_name ) {
		...
		$paths = array();
		$kikri_path=get_template_directory().'/lib/vendor/kirki';
		// Build the filename.
		...
		if ( isset( $name_parts[0] ) ) {
			...
			if ( 'Control' === $name_parts[0] || 'Settings' === $name_parts[0] ) {
				$path  = $kikri_path . '/controls/php/';
				$paths[] = $path . $filename;
			}
		}

		$paths[] = $kikri_path . '/core/' . $filename;
		$paths[] = $kikri_path . '/lib/' . $filename;
		...
		$previous_path = '';
		for ( $i = 0; $i < $levels; $i++ ) {
			$paths[] = $kikri_path. '/' . $previous_path . strtolower( $exploded[ $i ] ) . '/' . $filename;
